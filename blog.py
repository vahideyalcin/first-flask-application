...
def addarticle():
    ...
    if request.method == "POST" and form.validate():
        ...
        cursor = mysql.connection.cursor()
        sorgu = "Insert into articles(title,author,content,created_date) VALUES(%s,%s,%s,%s)"
        cursor.execute(sorgu,(title,session["username"],content,datetime.now()))
...
class ArticleForm(Form):
    ...
    content = TextAreaField("Makale İçeriği",validators=[validators.Length(max=10000)])
